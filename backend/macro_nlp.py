...
question_answerer_distil = pipeline("question-answering",
                             model = "distilbert-base-uncased")

# question_answerer_distil = TFDistilBertForQuestionAnswering.from_pretrained('distilbert-base-uncased')


# question_answerer_distil = pipeline("question_answering", 
                                    # model = "distilbert-base-uncased")
...
def product_question_distil(tags: List[str], question: str):
    """ get all tags of soup """
    results: Dict[str, int] = {}
    ...
    for tag in tags:
        if any(key_word in tag for key_word in question_key_words) and 40 < len(tag) < 1000:
            result = question_answerer_distil(question=question, context=tag)
            # result = { "answer": ..., "score": ..., ... }
